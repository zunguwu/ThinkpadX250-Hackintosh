</ul>
<article class="markdown-body entry-content container-lg" itemprop="text"><h2 tabindex="-1" dir="auto"><a id="user-content-specification" class="anchor" aria-hidden="true" tabindex="-1" href="#specification"><svg class="octicon octicon-link" viewBox="0 0 16 16" version="1.1" width="16" height="16" aria-hidden="true"><path d="m7.775 3.275 1.25-1.25a3.5 3.5 0 1 1 4.95 4.95l-2.5 2.5a3.5 3.5 0 0 1-4.95 0 .751.751 0 0 1 .018-1.042.751.751 0 0 1 1.042-.018 1.998 1.998 0 0 0 2.83 0l2.5-2.5a2.002 2.002 0 0 0-2.83-2.83l-1.25 1.25a.751.751 0 0 1-1.042-.018.751.751 0 0 1-.018-1.042Zm-4.69 9.64a1.998 1.998 0 0 0 2.83 0l1.25-1.25a.751.751 0 0 1 1.042.018.751.751 0 0 1 .018 1.042l-1.25 1.25a3.5 3.5 0 1 1-4.95-4.95l2.5-2.5a3.5 3.5 0 0 1 4.95 0 .751.751 0 0 1-.018 1.042.751.751 0 0 1-1.042.018 1.998 1.998 0 0 0-2.83 0l-2.5 2.5a1.998 1.998 0 0 0 0 2.83Z"></path></svg></a>Specification</h2>
<table>
<thead>
<tr>
<th>Specification</th>
<th>Information</th>
</tr>
</thead>
<tbody>
<tr>
<td>Processor</td>
<td>Intel Core i3-5010U</td>
</tr>
<tr>
<td>Integrated Graphics</td>
<td>Intel HD Graphics 5500</td>
</tr>
<tr>
<td>Memory</td>
<td>Samsung 8GB DDR3L</td>
</tr>
<tr>
<td>Storage</td>
<td>240GB PNY SSD Sata</td>
</tr>

<tr>
<td>Wireless Card</td>
<td>Intel AC-7265 Dual Band + Bluetooth</td>
</tr>
<tr>
<td>Bootloader</td>
<td>OpenCore</td>
</tr>
<tr>
<td>Bootloader Version</td>
<td>v1.0.7</td>
</tr>
</tbody>
</table>

<h2 tabindex="-1" dir="auto"><a id="user-content-screenshot" class="anchor" aria-hidden="true" tabindex="-1" href="#screenshot"><svg class="octicon octicon-link" viewBox="0 0 16 16" version="1.1" width="16" height="16" aria-hidden="true"><path d="m7.775 3.275 1.25-1.25a3.5 3.5 0 1 1 4.95 4.95l-2.5 2.5a3.5 3.5 0 0 1-4.95 0 .751.751 0 0 1 .018-1.042.751.751 0 0 1 1.042-.018 1.998 1.998 0 0 0 2.83 0l2.5-2.5a2.002 2.002 0 0 0-2.83-2.83l-1.25 1.25a.751.751 0 0 1-1.042-.018.751.751 0 0 1-.018-1.042Zm-4.69 9.64a1.998 1.998 0 0 0 2.83 0l1.25-1.25a.751.751 0 0 1 1.042.018.751.751 0 0 1 .018 1.042l-1.25 1.25a3.5 3.5 0 1 1-4.95-4.95l2.5-2.5a3.5 3.5 0 0 1 4.95 0 .751.751 0 0 1-.018 1.042.751.751 0 0 1-1.042.018 1.998 1.998 0 0 0-2.83 0l-2.5 2.5a1.998 1.998 0 0 0 0 2.83Z"></path></svg></a>Screenshot</h2>
<p align="center" dir="auto">
<kbd><br>S O N O M A 14.1.1
<br><br>
<kbd> <img src="https://i.imgur.com/CwgWQ1N.png" style="max-width: 100%;"></a></kbd></kbd>
<br><br>
</p><p align="center" dir="auto">
<kbd><br>About this Mac
<br><br>
<kbd><img src="https://i.imgur.com/qRBVo9Z.png" style="max-width: 100%;"></a></kbd></kbd>
</p>
<p align="center" dir="auto">
<kbd><br>Graphics Info
<br><br>
<kbd> <img src="https://i.imgur.com/u66Lhg9.png" style="max-width: 100%;"></a></kbd></kbd>
</p>


<h2 tabindex="-1" dir="auto"><a id="user-content-macos" class="anchor" aria-hidden="true" tabindex="-1" href="#macos"><svg class="octicon octicon-link" viewBox="0 0 16 16" version="1.1" width="16" height="16" aria-hidden="true"><path d="m7.775 3.275 1.25-1.25a3.5 3.5 0 1 1 4.95 4.95l-2.5 2.5a3.5 3.5 0 0 1-4.95 0 .751.751 0 0 1 .018-1.042.751.751 0 0 1 1.042-.018 1.998 1.998 0 0 0 2.83 0l2.5-2.5a2.002 2.002 0 0 0-2.83-2.83l-1.25 1.25a.751.751 0 0 1-1.042-.018.751.751 0 0 1-.018-1.042Zm-4.69 9.64a1.998 1.998 0 0 0 2.83 0l1.25-1.25a.751.751 0 0 1 1.042.018.751.751 0 0 1 .018 1.042l-1.25 1.25a3.5 3.5 0 1 1-4.95-4.95l2.5-2.5a3.5 3.5 0 0 1 4.95 0 .751.751 0 0 1-.018 1.042.751.751 0 0 1-1.042.018 1.998 1.998 0 0 0-2.83 0l-2.5 2.5a1.998 1.998 0 0 0 0 2.83Z"></path></svg></a>How to make it work</h2>
<ul dir="auto">
<li>Install Sonoma 14.1.1 using this EFI</li>
<li>If the installation won't proceed, on Device Properties, comment out the <code>PciRoot(0x0)/Pci(0x2,0x0)</code> by adding <b>#</b> infront of it</li> <br> <img src="https://i.imgur.com/BjYINrH.png" style="max-width: 100%;">
<li>Install <a href="https://github.com/dortania/OpenCore-Legacy-Patcher">OpenCore Legacy Patcher</a> and run the program to patch your x250</li>
<li>Once the installation of OLCP succeeded, uncomment out the <code>PciRoot(0x0)/Pci(0x2,0x0)</code> </li>
 <li>I disabled the itlwm (the file was to large for me to upload all the EFI folder, I need to remove him) for this configuration, add the itlwm kext in the Kexts folder and enable it on the config.plist. 
To download itlwm <a href="https://github.com/OpenIntelWireless/itlwm/releases"><code>click here</code></a></p>
<br><img src="https://i.imgur.com/Y4Nqz27.png" style="max-width: 100%;"> </li>
</ul>
<blockquote>
<p dir="auto">
</blockquote>
<h1 tabindex="-1" dir="auto"><a id="user-content-bios" class="anchor" aria-hidden="true" tabindex="-1" href="#bios"><svg class="octicon octicon-link" viewBox="0 0 16 16" version="1.1" width="16" height="16" aria-hidden="true"><path d="m7.775 3.275 1.25-1.25a3.5 3.5 0 1 1 4.95 4.95l-2.5 2.5a3.5 3.5 0 0 1-4.95 0 .751.751 0 0 1 .018-1.042.751.751 0 0 1 1.042-.018 1.998 1.998 0 0 0 2.83 0l2.5-2.5a2.002 2.002 0 0 0-2.83-2.83l-1.25 1.25a.751.751 0 0 1-1.042-.018.751.751 0 0 1-.018-1.042Zm-4.69 9.64a1.998 1.998 0 0 0 2.83 0l1.25-1.25a.751.751 0 0 1 1.042.018.751.751 0 0 1 .018 1.042l-1.25 1.25a3.5 3.5 0 1 1-4.95-4.95l2.5-2.5a3.5 3.5 0 0 1 4.95 0 .751.751 0 0 1-.018 1.042.751.751 0 0 1-1.042.018 1.998 1.998 0 0 0-2.83 0l-2.5 2.5a1.998 1.998 0 0 0 0 2.83Z"></path></svg></a>Bios Settings: Load the default settings</h1>
<ul dir="auto">

<li><code>Secure Boot -&gt; Secure Boot</code>: <strong>Disabled</strong>;</li>
<li><code>UEFI/Legacy Boot</code>: <strong>UEFI Only</strong>;</li>
<li><code>CSM Support</code>: <strong>No</strong>.</li>
</ul>

<h2 tabindex="-1" dir="auto"><a id="user-content-whats-working" class="anchor" aria-hidden="true" tabindex="-1" href="#whats-working"><svg class="octicon octicon-link" viewBox="0 0 16 16" version="1.1" width="16" height="16" aria-hidden="true"><path d="m7.775 3.275 1.25-1.25a3.5 3.5 0 1 1 4.95 4.95l-2.5 2.5a3.5 3.5 0 0 1-4.95 0 .751.751 0 0 1 .018-1.042.751.751 0 0 1 1.042-.018 1.998 1.998 0 0 0 2.83 0l2.5-2.5a2.002 2.002 0 0 0-2.83-2.83l-1.25 1.25a.751.751 0 0 1-1.042-.018.751.751 0 0 1-.018-1.042Zm-4.69 9.64a1.998 1.998 0 0 0 2.83 0l1.25-1.25a.751.751 0 0 1 1.042.018.751.751 0 0 1 .018 1.042l-1.25 1.25a3.5 3.5 0 1 1-4.95-4.95l2.5-2.5a3.5 3.5 0 0 1 4.95 0 .751.751 0 0 1-.018 1.042.751.751 0 0 1-1.042.018 1.998 1.998 0 0 0-2.83 0l-2.5 2.5a1.998 1.998 0 0 0 0 2.83Z"></path></svg></a>What's Working?</h2>
<ul dir="auto">
<li>QE/CI Intel HD Graphics 5500 <code>I think because of the OpenCore Legacy Patch</code></li>
<li>Power Management</li>
<li>Sleep, Shutdown, Restart </li>
<li>Audio Speaker &amp; Earphone </li>
<li>Trackpad, Trackball, Gestures </li>
<li>Battery Indicator</code></li>
</ul>

<h2 tabindex="-1" dir="auto"><a id="user-content-whats-working" class="anchor" aria-hidden="true" tabindex="-1" href="#whats-working"><svg class="octicon octicon-link" viewBox="0 0 16 16" version="1.1" width="16" height="16" aria-hidden="true"><path d="m7.775 3.275 1.25-1.25a3.5 3.5 0 1 1 4.95 4.95l-2.5 2.5a3.5 3.5 0 0 1-4.95 0 .751.751 0 0 1 .018-1.042.751.751 0 0 1 1.042-.018 1.998 1.998 0 0 0 2.83 0l2.5-2.5a2.002 2.002 0 0 0-2.83-2.83l-1.25 1.25a.751.751 0 0 1-1.042-.018.751.751 0 0 1-.018-1.042Zm-4.69 9.64a1.998 1.998 0 0 0 2.83 0l1.25-1.25a.751.751 0 0 1 1.042.018.751.751 0 0 1 .018 1.042l-1.25 1.25a3.5 3.5 0 1 1-4.95-4.95l2.5-2.5a3.5 3.5 0 0 1 4.95 0 .751.751 0 0 1-.018 1.042.751.751 0 0 1-1.042.018 1.998 1.998 0 0 0-2.83 0l-2.5 2.5a1.998 1.998 0 0 0 0 2.83Z"></path></svg></a>What's NOT Working?</h2>
<ul dir="auto">
<li>iMessage and Facetime <code>I think because of the itlwm has still some glitch, they are still developing this kext</code></li>
<li>Camera <code>I cannot tell, I dont have a webcam or its malfunctioning ever since I bought this 2nd hand</code></li>
<li>Don't expect a smooth usage of Sonoma on this x250, by paper it will run well on Monterey, while using OLCP will make your machine run Sonoma on it, patching it means it will bypass some of the macos codes/settings </li>
</ul>


<h2 tabindex="-1" dir="auto"><a id="user-content-credits" class="anchor" aria-hidden="true" tabindex="-1" href="#credits"><svg class="octicon octicon-link" viewBox="0 0 16 16" version="1.1" width="16" height="16" aria-hidden="true"><path d="m7.775 3.275 1.25-1.25a3.5 3.5 0 1 1 4.95 4.95l-2.5 2.5a3.5 3.5 0 0 1-4.95 0 .751.751 0 0 1 .018-1.042.751.751 0 0 1 1.042-.018 1.998 1.998 0 0 0 2.83 0l2.5-2.5a2.002 2.002 0 0 0-2.83-2.83l-1.25 1.25a.751.751 0 0 1-1.042-.018.751.751 0 0 1-.018-1.042Zm-4.69 9.64a1.998 1.998 0 0 0 2.83 0l1.25-1.25a.751.751 0 0 1 1.042.018.751.751 0 0 1 .018 1.042l-1.25 1.25a3.5 3.5 0 1 1-4.95-4.95l2.5-2.5a3.5 3.5 0 0 1 4.95 0 .751.751 0 0 1-.018 1.042.751.751 0 0 1-1.042.018 1.998 1.998 0 0 0-2.83 0l-2.5 2.5a1.998 1.998 0 0 0 0 2.83Z"></path></svg></a>Credits:</h2>
<ul dir="auto">
<li><a href="https://github.com/zamkara">zamkara</a> for the EFI that I used and modified to make it work on Sonoma, as well as the description code on this page</li>
</ul>
<h2 tabindex="-1" dir="auto"><a id="user-content-download" class="anchor" aria-hidden="true" tabindex="-1" href="#download"><svg class="octicon octicon-link" viewBox="0 0 16 16" version="1.1" width="16" height="16" aria-hidden="true"><path d="m7.775 3.275 1.25-1.25a3.5 3.5 0 1 1 4.95 4.95l-2.5 2.5a3.5 3.5 0 0 1-4.95 0 .751.751 0 0 1 .018-1.042.751.751 0 0 1 1.042-.018 1.998 1.998 0 0 0 2.83 0l2.5-2.5a2.002 2.002 0 0 0-2.83-2.83l-1.25 1.25a.751.751 0 0 1-1.042-.018.751.751 0 0 1-.018-1.042Zm-4.69 9.64a1.998 1.998 0 0 0 2.83 0l1.25-1.25a.751.751 0 0 1 1.042.018.751.751 0 0 1 .018 1.042l-1.25 1.25a3.5 3.5 0 1 1-4.95-4.95l2.5-2.5a3.5 3.5 0 0 1 4.95 0 .751.751 0 0 1-.018 1.042.751.751 0 0 1-1.042.018 1.998 1.998 0 0 0-2.83 0l-2.5 2.5a1.998 1.998 0 0 0 0 2.83Z"></path></svg></a>The easiest guide to install macOS on your computer</h2>
</article>
